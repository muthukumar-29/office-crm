...
public class PdfGeneratorUtil {
    ...
    public byte[] generateCertificatePdf(Certificate cert) throws DocumentException {
        ...
        if (cert.getCollegeName() != null)
            addCenter(doc,
                    (cert.getRollNo() != null ? cert.getRollNo() + "  |  " : "") + cert.getCollegeName(),
                    subFont, 4f);

        String catStr = cert.getCategory() != null ? cert.getCategory() : "COURSE";
        String label;
        if ("INTERN".equals(catStr))       label = "has successfully completed the internship program:";
        else if ("PROJECT".equals(catStr)) label = "has successfully delivered the project:";
        else                               label = "has successfully completed the course:";
    ...
    public byte[] generateInvoicePdf(Invoice inv) throws DocumentException {
        ...
        compCell.setBackgroundColor(PRIMARY);
        ...
        invCell.setBackgroundColor(GOLD);
        ...
        invCell.addElement(new Phrase("# " + safe(inv.getInvoiceNumber()), whtS));
        invCell.addElement(new Phrase("Date: " + (inv.getInvoiceDate() != null ? inv.getInvoiceDate() : ""), whtS));
        if (inv.getDueDate() != null)
            invCell.addElement(new Phrase("Due: " + inv.getDueDate(), whtS));
        ...
        if (inv.getClientName() != null && !inv.getClientName().isBlank()) {
            ...
            bill.setWidthPercentage(45);
            bill.setHorizontalAlignment(Element.ALIGN_LEFT);
            PdfPCell bc = new PdfPCell();
            bc.setBackgroundColor(LIGHT);
            bc.setPadding(10);
        ...
        if (inv.getItems() != null && !inv.getItems().isEmpty()) {
            ...
            for (InvoiceItem item : inv.getItems()) {
                ...
                addItemCell(items, safe(item.getDescription()),      td, bg, Element.ALIGN_LEFT);
                ...
                // getTotalPrice() — field renamed from 'amount' to 'totalPrice' in InvoiceItem
        ...
        BigDecimal subtotal   = orZero(inv.getSubtotal());
        BigDecimal discount   = orZero(inv.getDiscount());
        BigDecimal taxPercent = orZero(inv.getTaxPercent());
        BigDecimal taxAmount  = orZero(inv.getTaxAmount());
        BigDecimal total      = orZero(inv.getTotalAmount());
        BigDecimal balanceDue = orZero(inv.getBalanceDue());

        addTotalRow(totals, "Subtotal:",   "Rs. " + subtotal,   label, val,   BaseColor.WHITE);

        if (discount.compareTo(BigDecimal.ZERO) > 0)
            addTotalRow(totals, "Discount:", "- Rs. " + discount, label, val, BaseColor.WHITE);

        if (taxPercent.compareTo(BigDecimal.ZERO) > 0)
            addTotalRow(totals, "Tax (" + taxPercent + "%):", "Rs. " + taxAmount, label, val, BaseColor.WHITE);

        addTotalRow(totals, "Total:", "Rs. " + total, grand, grand, PRIMARY);
        addTotalRow(totals, "Balance Due:", "Rs. " + balanceDue, label,
        ...
        if (inv.getNotes() != null && !inv.getNotes().isBlank()) {
            doc.add(Chunk.NEWLINE);
            doc.add(new Paragraph("Note: " + inv.getNotes(),
                    new Font(Font.FontFamily.HELVETICA, 10, Font.ITALIC, MUTED)));
        }

        doc.add(Chunk.NEWLINE);
        doc.add(new Chunk(new LineSeparator(1, 100, GOLD, Element.ALIGN_CENTER, -5)));
        Paragraph foot = new Paragraph("Thank you for your business! — Computer generated invoice.",
                new Font(Font.FontFamily.HELVETICA, 9, Font.ITALIC, MUTED));
    ...
    private void drawBorder(PdfContentByte c, Document doc) {
        ...
        c.rectangle(m, m, page.getWidth() - 2 * m, page.getHeight() - 2 * m);
        c.stroke();
        c.setColorStroke(GOLD); c.setLineWidth(1.5f);
        c.rectangle(m + 6, m + 6, page.getWidth() - 2 * (m + 6), page.getHeight() - 2 * (m + 6));
        c.stroke();
    }
    ...
    private void addTotalRow(PdfPTable t, String lbl, String v,
                             Font lFont, Font vFont, BaseColor bg) {
        PdfPCell l = new PdfPCell(new Phrase(lbl, lFont));
        l.setPadding(5); l.setBorder(Rectangle.NO_BORDER); l.setBackgroundColor(bg);
        PdfPCell r = new PdfPCell(new Phrase(v,   vFont));
        r.setPadding(5); r.setBorder(Rectangle.NO_BORDER);
        r.setHorizontalAlignment(Element.ALIGN_RIGHT); r.setBackgroundColor(bg);
        t.addCell(l);
        t.addCell(r);
    }
